...
describe("ERC20 New Bid Tests", function () {
  ...
  describe("Test make bids and bid requirements", function () {
    ...
    it("Calling makeBid to bid on new NFTAuction with ERC20", async function () {
      await nftAuction
        .connect(user2)
        .makeBid(erc1155.address, tokenId, auctionId, erc20.address, minPrice);
    ...
    it("should not allow bidder to withdraw if min price exceeded", async function () {
      await nftAuction
        ...
        .makeBid(erc1155.address, tokenId, auctionId, erc20.address, minPrice);
      await expect(
        nftAuction.connect(user2).withdrawBid(erc1155.address, tokenId, auctionId)
    ...
    it("should not allow user to set 0 tokenAmount and 0 Eth amount", async function () {
      await expect(
        nftAuction
          .connect(user2)
          .makeBid(erc1155.address, tokenId, auctionId, erc20.address, zeroERC20Tokens)
      ).to.be.revertedWith("Bid to be in specified ERC20/Eth");
    });
    ...
    it("should allow seller take highestbid and conclude auction", async function () {
      ...
      await nftAuction.connect(user1).takeHighestBid(erc1155.address, tokenId, auctionId);
      ...
      expect(await erc1155.balanceOf(user3.address, tokenId));
    });
    it("should allow seller update buyNowPrice and conclude auction", async function () {
      ...
      await nftAuction
        .connect(user1)
        .updateBuyNowPrice(erc1155.address, tokenId, auctionId, bidIncreaseByMinPercentage);
      result = await nftAuction.nftContractAuctions(erc1155.address, tokenId, auctionId);
      expect(result.auctionEnd).to.be.equal(BigNumber.from(0));
      expect(await erc1155.balanceOf(user3.address, tokenId));
    });
  ...
  describe("Test underbid functionality", function () {
    ...
    beforeEach(async function () {
      ...
      await nftAuction
        .connect(user2)
        .makeBid(erc1155.address, tokenId, auctionId, erc20.address, underBid);
      result = await nftAuction.nftContractAuctions(erc1155.address, tokenId, auctionId);
    ...
    it("should reset bids when bidder withdraws funds", async function () {
      await nftAuction.connect(user2).withdrawBid(erc1155.address, tokenId, auctionId);
      result = await nftAuction.nftContractAuctions(erc1155.address, tokenId, auctionId);
    ...
    it("should not allow other users to withdraw underbid", async function () {
      await expect(
        nftAuction.connect(user3).withdrawBid(erc1155.address, tokenId, auctionId)
    ...
    it("should allow seller to update price above current bid and not start the auction", async function () {
      await nftAuction
        .connect(user1)
        .updateMinimumPrice(erc1155.address, tokenId, auctionId, underBid + 5);
      let result = await nftAuction.nftContractAuctions(
        erc1155.address,
        tokenId,
        auctionId
